...
public class BeautifyMultiThread {
    ...
    public class FilterTask implements  Runnable{
        ...
        public void run() {
            ...
            sharpen(res,src,width,height,4,1.5f,startRaw,endRaw);
        }
    }
    ...
    //均值滤波的锐化算法
    public int[] sharpen(int[] src,int[] res,int width,int height,int radius,float k,int startRaw,int endRaw){
        ...
        for(int i=startRaw;i<=endRaw;i++) {
            ...
            new_r= range((int) (mean_r+k*(old_r-mean_r)));
            new_g= range((int) (mean_g+k*(old_g-mean_g)));
            new_b= range((int) (mean_b+k*(old_b-mean_b)));
